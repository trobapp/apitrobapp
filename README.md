# apitrobapp
Versión de la API de Trobapp corporativo V 1.8.

Está disponible para los usuarios de Trobapp corporativo.  
Todos los puntos finales tienen el prefijo http://corporativo.trobapp/api/[endpoint]

Todos los puntos finales requieren un token de acceso API válido. Aprenda cómo obtener un token de acceso aquí . La API de trobapp actualmente no es compatible con CORS. Para evitar la exposición de su token de acceso, no recomendamos utilizar nuestra API del lado del cliente/en el navegador. En su lugar, use nuestra API del lado del servidor o considere usar un proxy.  

La API generalmente devuelve un resultado JSON, pero todos los puntos finales también admiten la .xmlextensión.
...
GET/insert.php
http://corporativo.trobapp/api/insert.php

* uniqueId - IMEI del dispositivo
* protocol - Protocolo del dispositivo
* altitude - Altitud actual del dispositivo en metros
* course -   Angulo del dispositivo actual de 0 a 360
* latitude - Latitud actual del dispositivo
* longitude - Longitud actual del dispositivo
* speed - Velocidad actual del dispositivo en unidades de nudos (https://en.wikipedia.org/wiki/Knot_(unit))
* fixTime - Hora actual del dispositivo en marca de tiempo con milisegundos
* attributes - clave: valore sus otros parámetros

*Versión actual: [v1.8.0][dist]*
